#Pantalla inicio
def render_home_aux(screen, background, font, screen_height, options, x, y, index, hovered_index):
    ...
    return render_home_aux(screen, background, font, screen_height, options, x, y, index + 1, hovered_index)

def render_home(screen, background, font, WIDTH, HEIGHT, hovered_index, options):
    screen.blit(background, (0, 0))

    # Calcular coordenada y para insertar texto
    y = (HEIGHT/ 3) - font.size(options[0])[1]

    # Calcular coordenada x para todas opciones
    x= WIDTH/2

    #Renderizar opciones
    return render_home_aux(screen, background, font, HEIGHT, options, x, y, 0, hovered_index)
